...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                Toast.remove(loadingId);
                Toast.error('❌ Cannot connect to server. Make sure it is running.');
            }
        });
    }
    ...
    if (registerLink) {
        registerLink.addEventListener('click', async function (e) {
            e.preventDefault();
            const username = prompt('Choose a username (3-30 alphanumeric chars):');
    ...
    if (findTrainsBtn) {
        findTrainsBtn.addEventListener('click', async function () {
            ...
            // Validate stations
            const stationValidation = Validators.validateStations(from, to);
            ...
            // Validate date
            const dateValidation = Validators.validateJourneyDate(date);
            ...
            // Validate passengers
            const passengersValidation = Validators.validatePassengers(passengers);
            if (!passengersValidation.valid) {
                Toast.error(passengersValidation.error);
                return;
            }
            ...
            const loadingId = Toast.loading('Searching for trains...');
            ...
            try {
                ...
                if (data.success) {
                    if (data.count === 0) {
                        Toast.warning('No trains found for this route. Try a different route.');
                    } else {
                        Toast.success(`Found ${data.count} train(s)`);
                        showTrainResults(data.trains, from, to, date, passengers);
                    }
                } else { 
                    Toast.error('❌ ' + data.message);
                }
    ...
    window.submitBooking = async function (trainId, selectedClass, fare, passengers, journeyDate) {
        ...
        // Validate using Validators utility
        const nameValidation = Validators.validatePassengerName(name);
        if (!nameValidation.valid) { Toast.error(nameValidation.error); return; }

        const cnicValidation = Validators.validateCNIC(cnic);
        if (!cnicValidation.valid) { Toast.error(cnicValidation.error); return; }
        ...
        const loadingId = Toast.loading('Processing your booking...');
    ...
    if (feedbackForm) {
        feedbackForm.addEventListener('submit', async function (e) {
            ...
            // Validate using Validators utility
            const feedbackValidation = Validators.validateFeedback(email, message, rating);
            if (!feedbackValidation.valid) {
                Toast.error(feedbackValidation.error);
                return;
            }
            ...
            const loadingId = Toast.loading('Submitting your feedback...');
    ...
    function showAlert(message, type) {
        ...
        // Remove 'info' prefix from loading messages
        const cleanMessage = message.replace('Logging in...', 'Processing...').replace('Registering...', 'Processing...').replace('Searching...', 'Searching trains...');
